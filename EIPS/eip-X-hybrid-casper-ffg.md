...
* `MIN_DEPOSIT_SIZE`: 1e20 wei (100 ETH)
...
`MIN_DEPOSIT_SIZE` is set to 100 ETH to help form a natural upper bound on the total number of validators, bounding the overhead due to `vote` messages. Due to contract mechanics this value is only the base minimum. The true minimum at any given time is defined by `casper.current_min_deposit_size()` which returns `max(MIN_DEPOSIT_SIZE, num_validators)` such that the true minimum scales as the number of active validators scales.
...
##### Join Fork
This setting is to be used by new clients that are syncing the network. Due to weak subjectivity, a blockhash should be supplied to successfully sync the network when initially starting a node.
